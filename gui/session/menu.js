...
function openDiplomacy()
{
	...
	for (var i = 1; i < players.length; i++)
	{
		...
		if (i == we || players[we].state != "active" || players[i].state != "active")
		{
			...
			for each (var a in ["TributeFood", "TributeWood", "TributeStone", "TributeMetal","TributeBlood", "Ally", "Neutral", "Enemy"])
				getGUIObjectByName("diplomacyPlayer"+a+"["+(i-1)+"]").hidden = true;
			continue;
		}

		// Tribute
		for each (var resource in ["food", "wood", "stone", "metal","blood"])
		{
			...
			button.onpress = (function(player, resource, button){
				...
				return function() {
					...
					var amounts = {
						...
						"blood": (resource == "blood" ? 100 : 0) * multiplier,
					};
